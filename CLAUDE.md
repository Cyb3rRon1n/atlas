...
**Discovery vs. plugins — still two separate commands, one shared store.** `atlas.discovery.run_discovery()` (`atlas/discovery/__init__.py`) is the built-in, synchronous path used by `atlas discover` — it directly calls `collect_system/hardware/storage/network` and returns a merged dict. Separately, `atlas/plugins/` implements a plugin architecture (`AtlasPlugin` ABC with `initialize(runtime)` / `discover()`, `PluginManager.load_plugins()` auto-discovers via `atlas/plugins/loader.py`, `discover_all()` runs every registered plugin) — this is what `atlas plugins` / `atlas discover-plugins` exercise. `atlas discover-plugins` now writes its results into `AtlasEnvironmentContext.containers` (keyed by plugin name) and persists/publishes them the same way `atlas discover` and `atlas proxmox scan` do, so `atlas analyze` can see plugin-sourced data too. What's still not unified: `atlas discover` does not itself run plugins — they remain two commands you run independently, not one merged discovery pass.
