...
| `atlas analyze` | Analyze the latest environment snapshot with AI — using live tool calls for current state — and print a summary plus recommendations (see [Configuration](configuration.md#intelligence)). `--json` prints `{"summary", "recommendations", "plan", "provider", "model"}` instead - never offers to run a suggested plan (no bypass flag), that's text-mode-only. No environment yet prints `null`, exit 0. Exits 1 on an `AIProviderError`, otherwise 0 - the one JSON-output command in this project with a real exit code for something other than reachability, since analysis has a genuine health concept `report`/`trends` don't. |
