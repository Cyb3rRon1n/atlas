...
- **Verification against real infrastructure.** Proxmox, both AI providers (Anthropic/Ollama), and Prometheus are real, tested code — but every test so far has been against mocks, never the actual live systems, since none existed in the development environment. This is tracked as its own milestone rather than left implicit: it gets checked off integration-by-integration as each is actually run against real hardware, distinct from "built and unit-tested."
- **`atlas doctor` readiness checks** — currently checks Python/memory/storage/Docker/inventory, but has no idea whether `proxmox.enabled` points at a reachable host, whether `ANTHROPIC_API_KEY` is set, or whether Prometheus is reachable. Extending it to check the three unverified integrations above would let `atlas doctor` answer "is this ready to use against my real setup" directly, instead of that being a manual read-the-code exercise.
